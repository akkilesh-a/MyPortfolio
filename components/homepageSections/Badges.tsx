...
export default function Badges() {
  const [active, setActive] = useState<(typeof badgesData)[number] | boolean | null>(
    null
  );
  const id = useId();
    ...
    <VortexWrapper otherClasses='w-screen px-8 py-8 space-y-16'>
        ...
        <AnimatePresence>
            ...
            <div className="fixed inset-0  grid place-items-center z-[100]">
                ...
                >
                <motion.div layoutId={`image-${active.title}-${id}`} className='p-3'>
                    <img
                    width={200}
                    height={200}
                    src={active.src}
                    alt={active.title}
                    className="w-full h-80 lg:h-80 object-scale-down"
                    />
                </motion.div>
